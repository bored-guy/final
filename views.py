...
def information(request):
    ...
    if request.POST:
        ...
        stu = Student(name=name, sex=sex, telephone=phone, email=email, qq=qq, id_number=id_number, school_name=school,
                      ...
                      belongs_team=identify_code, belongs_user=identify_code, if_room=if_room, state=path_name,if_paid=True,if_email=False)
        stu.save()
...
def get_data(request):
    if request.POST:
        ...
        stu = Student(name=name, sex=sex, telephone=phone, email=email, qq=qq, id_number=id_number, school_name=school,
                      school_name_in_english=school_in_english, identify_code=identify_code, place=place,
                      belongs_team="No", belongs_user=identify_code, if_room=if_room, if_paid=False,if_email=False)
        stu.save()
        user = User.objects.filter(identify_code=identify_code, if_yanzheng=True)[0]
        user.if_person = True
...
def mail(request):
    while True:
        ...
        the_team_fenpei=Team.objects.filter(if_email_fenpei=True)
        the_team_money = Team.objects.filter(if_email_money=True)
        for i in the_team_fenpei:
            identify_code = i.identify_code
            the_user = User.objects.filter(identify_code=identify_code,if_yanzheng=True)[0]
            email = the_user.email
            string = ""
            string += "WESMUN组委会已为您分配名额，"
            string += "安全理事会" +str(i.anli_num)+"人，"
            string += "世界卫生组织" + str(i.shiwei_num)+"人，"
            string += "历史委员会" + str(i.San_num)+"人，"
            string += "特殊委员会：哈利波特联动委员会" + str(i.harry_num) + "人，"
            string += "特殊委员会：1956西迁" + str(i.xiqian_num) + "人，"
            string += "经社理事会" + str(i.jingshe_num) + "人，"
            string += "主新闻中心" + str(i.news_num) + "人，"
            send_mail('WESMUN组委会已为您分配名额',string, '2232652509@qq.com', [email], fail_silently=False)
            print("向"+email+"发送邮件"+"内容为"+string)
            i.if_email_xiwei=False
            i.save()
        for i in the_team_money:
            identify_code = i.identify_code
            the_user = User.objects.filter(identify_code=identify_code,if_yanzheng=True)[0]
            email = the_user.email
            string = "WESMUN组委会已经确认收到您的缴费"
            send_mail('WESMUN组委会已经确认收到您的缴费', string, '2232652509@qq.com', [email], fail_silently=False)
            print("向" + email + "发送邮件" + "内容为" + string)
            i.if_email_money = False
            i.save()

        the_stu_xiwei = Student.objects.filter(if_email_xiwei=True)
        the_stu_money = Student.objects.filter(if_email_money=True)
        for i in the_stu_money:
            identify_code = i.identify_code
            the_user = User.objects.filter(identify_code=identify_code, if_yanzheng=True)[0]
            email = the_user.email
            send_mail('WESMUN组委会已确认您的缴费', 'WESMUN组委会已确认您的缴费', '2232652509@qq.com', [email], fail_silently=False)
            print("向" + email + "发送邮件" + "内容为" + 'WESMUN组委会已确认您的缴费')
            i.if_email = False
            i.save()
        for i in the_stu_xiwei:
            identify_code = i.identify_code
            the_user = User.objects.filter(identify_code=identify_code, if_yanzheng=True)[0]
            email = the_user.email
            the_dict = {1:"安全理事会",2: "世界卫生组织", 3: "历史委员会", 4: "特殊委员会：哈利波特联动委员会",
            5: "特殊委员会：1956西迁", 6: "经社理事会", 7: "主新闻中心"}
            send_mail('WESMUN组委会已确认您的席位', '您的席位为'+the_dict[i.place], '2232652509@qq.com', [email], fail_silently=False)
            print("向" + email + "发送邮件" + "内容为" + '您的席位为'+the_dict[i.place])
            i.if_email_xiwei = False
            i.save()
        print("60s过去了")
        time.sleep(60)
